...
public class ServiceAnnoncef {
    ...
    public boolean addAnnoncef(Annoncef l) {
        String url = Statics.BASE_URL + "/annoncef/AddjsonA?nomf=" + l.getNomf() + "&adresse=" + l.getAdresse() + "&emailf=" + l.getEmailf() + "&descf=" + l.getDescf();
        req.setUrl(url);
        req.setPost(false);
...
public ArrayList<Annoncef> searchAnnoncesByName(String name) {
    ArrayList<Annoncef> annonces = getAllAnnonces(); // récupérer toutes les annonces depuis le serveur
    ArrayList<Annoncef> result = new ArrayList<>();
    for (Annoncef annonce : annonces) {
        if (annonce.getNomf().toLowerCase().contains(name.toLowerCase())) {
            result.add(annonce);
        }
    }
    return result;
